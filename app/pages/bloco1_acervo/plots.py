...
def _fig_1b2_base(df: pd.DataFrame, esconder_eixo_y: bool = False) -> tuple[go.Figure, list[str], pd.Series]:
    ...
    fig = aplicar_padrao(
        ...
        height=650, margin=dict(t=185, b=120, l=60, r=40),
    )
...
def fig_1b3_acervo_por_classe_vertical_extremos(df: pd.DataFrame, show_values: bool = True) -> go.Figure:
    ...
    for ano in ("2017", "2025"):
        if ano in anos:
            i = anos.index(ano)
            fig.add_annotation(x=i, y=totais.iloc[i], yanchor="bottom", yshift=4,
                               text=f"<b>{br(totais.iloc[i])}</b>", showarrow=False,
                               font=dict(color="black", size=14), bgcolor="white", borderpad=2,
                               xref="x", yref="y")
    return fig
...
def fig_1d_variacao_anual(df: pd.DataFrame, show_values: bool = True) -> go.Figure:
    ...
    fig = go.Figure(go.Bar(x=anos_str, y=tot["variacao"], marker_color=cores, showlegend=False,
                           ...
                           cliponaxis=False))
    # Legend dummy
    for nome, cor in [("ACRÉSCIMO", CINZA), ("DECRÉSCIMO", VERMELHO)]:
        fig.add_trace(go.Bar(x=[None], y=[None], name=nome, marker_color=cor, showlegend=True))
    v_abs = max(abs(tot["variacao"].max()), abs(tot["variacao"].min()))
    ymax = int(v_abs * 1.4)
    ymin = -int(v_abs * 1.15)
