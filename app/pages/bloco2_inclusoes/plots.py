...
def _classe_ano(df: pd.DataFrame, ambiente: str, show_values: bool, titulo: str, subtitulo: str, show_pct: bool = False) -> go.Figure:
    ...
    # altura da barra = % do total do ano (não valor absoluto) — assim 2.e e 2.f ficam
    # visualmente equivalentes em proporção, mesmo com volumes totais bem diferentes.
    pct_tab = tab.div(totais_ano, axis=0).fillna(0) * 100
    ...
    for classe in _CLASSES:
        if show_values:
            ...
            textos = None
        fig.add_trace(go.Bar(
            x=anos, y=pct_tab[classe], name=classe, marker_color=_CORES_CLASSE[classe],
    ...
    fig.update_yaxes(range=yrange, showline=False, showticklabels=False, ticks="")
    fig.update_xaxes(tickfont=dict(size=22), title_font=dict(size=22))
    return fig
...
def fig_2l2_pauta_vs_concluidos(df: pd.DataFrame, show_values: bool = True) -> go.Figure:
    ...
    fig.update_yaxes(showline=False, showticklabels=False, ticks="")
    fig.update_xaxes(tickfont=dict(size=20), title_font=dict(size=20), ticks="")
    return fig
...
def _categoria_ano(df: pd.DataFrame, ambiente: str, show_values: bool, titulo: str, subtitulo: str,
                   legend_y: float = 1.05, ymax: float | None = None, use_pct: bool = False) -> go.Figure:
    cats = ["Unânime", "Maioria (relator vencedor)", "Maioria (relator vencido)", "Não concluído"]
    ...
    bar_y = pct_tab if use_pct else tab
    yrange = [0, 105] if use_pct else [0, (ymax if ymax is not None else tab.values.max()) * 1.15]
    ...
    for cat in cats:
        if show_values:
            ...
            textos = None
        fig.add_trace(go.Bar(
            x=anos, y=bar_y[cat], name=cat.upper(), marker_color=_CORES_CATEGORIA[cat],
            text=textos, textposition="outside", textfont=dict(color="black", size=20, weight="bold"),
            cliponaxis=False,
        ))
    fig = aplicar_padrao(
...
def fig_2n2_categoria_ano_pp(df: pd.DataFrame, show_values: bool = True) -> go.Figure:
    return _categoria_ano(df, "Plenário Presencial", show_values,
                           "Desfecho por categoria e ano — Plenário Presencial (2020–2025)", None,
                           legend_y=1.2, use_pct=True)
...
def _tramitacao_periodo(df: pd.DataFrame, ano_ini: int, ano_fim: int, show_values: bool,
    ...
    for cat in vc.index:
        fig.add_trace(go.Bar(
            y=[cat], x=[pct[cat]], orientation="h", name=cat.replace("Somente ", "").upper(),
            marker_color=_CORES_TRAMITACAO[cat],
    ...
    fig = aplicar_padrao(
        fig, titulo, subtitulo,
        xaxis=dict(title="", range=[0, 100], showticklabels=False, showline=False, ticks=""),
        yaxis=dict(title="", showticklabels=False, showline=False, ticks="",
                   categoryorder="array", categoryarray=ordem_topo_base),
        height=420, showlegend=True,
        margin=dict(t=230, b=70, l=40, r=40),
    )
    fig.update_layout(legend=dict(orientation="h", yanchor="bottom", y=1.30, x=0.5, xanchor="center",
                                   font=dict(size=15)))
